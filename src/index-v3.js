...
function setupPythonRPC(){
	exec(process.cwd()+'/src/python/dist/cycler_wrapper/cycler_wrapper_rpc.exe', function (err, results) {
		if (err != null){
			console.log(err);
		}
		else {
			...
			fs.writeFile(info.path, JSON.stringify(preview_data), function(err) {
				if (!err){
					temp.open({suffix:'.png'}, function(err2,info2) {
						...
						exec(process.cwd()+'/src/python/dist/cycler_wrapper/cycler_wrapper.exe', args, {maxBuffer:1024*8192}, function (err, results) {
							if (err != null){
								console.log(err)
							}
							results = JSON.parse(results);
			...
			fs.writeFile(info.path, JSON.stringify(file_data_dict[fn]), function(err){
				if (!err){
					temp.open({suffix:'.png'}, function(err2,info2){
						...
						exec(process.cwd()+'/src/python/dist/cycler_wrapper/cycler_wrapper.exe', args, {maxBuffer:1024*4096}, function (err, results) {
							if (err != null){
								console.log(err)
							}
